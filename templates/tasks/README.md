...
Runs the `default` task (`tasks/register/default.iced.md`).

###### `sails lift --prod`

Runs the `prod` task (`tasks/register/prod.iced.md`).

###### `sails www`

Runs the `build` task (`tasks/register/build.iced.md`).

###### `sails www --prod` (production)

Runs the `buildProd` task (`tasks/register/buildProd.iced.md`).


### Can I customize this for SASS, Angular, client-side Jade templates, etc?

You can modify, omit, or replace any of these Grunt tasks to fit your requirements. You can also add your own Grunt tasks - just add a `someTask.js` file in the `grunt/config` directory to configure the new task, then register it with the appropriate parent task(s) (see files in `grunt/register/*.iced.md`).
